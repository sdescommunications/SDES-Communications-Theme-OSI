...
const ORGINFO = [
  {
    name: 'Late Knights',
    info:
      'Late Knights hosts monthly events that features free food, prizes, and entertainment. The goal of Late Knights is to provide a space that is fun, safe, on-campus, and free of drugs and alcohol. Events are held on campus from 10 p.m. to 1 a.m.',
...

  {
    ...
    info:
      'CAB is responsible for the planning and execution of various events on the UCF campus. Our events include large-scale concerts and comedy shows, speaking engagements, the Mr. and Miss UCF Scholarship Pageants, movie screenings, and much more!',
...

  {
    ...
    info:
      'KOTC focuses on serving as a bridge between connection campuses to the Orlando campus. It is dedicated to helping students “Get Involved” while providing quality programs and services at each of UCF’s connector campuses',
...

  {
    ...
    info:
      'Student Engagement is a partnership with OSI, SJA and Valencia College to provide programming, services, student organizations, and student leadership opportunities at the Downtown Campus.',
...

  {
    name: 'Homecoming',
    info:
      'Homecoming is a fun week that celebrates campus traditions for the campus community. Come and celebrate the black and gold spirit by participating in traditions like: Comedy Knight, Concert Knight, Movie Knight, Knights Got Talent, Spirit Splash, Fireworks Festival, Greek Extravaganza Step Show, and many more.',
...

  {
    ...
    info:
      'OSI Productions provides high-quality video, photo, and livestream content to various departments and agencies across campus.',
...

  {
    ...
    info:
      'Knights of the Turntables (KoT) aims to provide the UCF community a fun, low-cost way of bringing DJ, music, and MC services to live events.',
...

  {
    ...
    info:
      'A professional recording studio available to UCF students. Students can record podcasts, music, and voiceovers based on availability.',
...

  {
    ...
    info:
      'Graduate Outreach provides programming, events, and services for Graduate students, connecting students with alumni, faculty, and the community. Events include Monthly First Wednesday Cafe, Mocktail, LinkedIn Photoshoots, and Grad Gathering.',
...

  {
    name: 'Design Group',
    info:
      'Design Group is the team of students that create graphics, illustrations, and promotional material for the Office of Student Involvement. We also handle web design and development for OSI, Student Government, and other organizations. This includes building fun projects like this quiz you just played. :D',
...
var Quiz = function () {
  ...
  this._showResult = function () {
    ...
    $resultBox.html(
      "<h1 class='mb-4'><p>Here is your result!</p> <p>You matched with <i>" +
        ORGINFO[resultList[0]].name +
        '</i></p> </h1> <p>' +
        ORGINFO[resultList[0]].info +
        '</p><p class="text-center"><a class="my-3 mx-auto btn btn-warning btn-lg" href="' +
        ORGINFO[resultList[0]].link +
        '"target="_blank"><strong>Visit Website</strong></a></p>'
    );
    ...
    const resultToString = () => {
      ...
      for (let i = 1; i < resultList.length; i++) {
        myStr +=
          '<li ><a class="collapsed" href="#collapseInfo-' +
          i +
          '"data-toggle="collapse" role="button" aria-expanded="false" id="info-' +
          i +
          '"><h4><strong><u>' +
          ORGINFO[resultList[i]].name +
          '</u></strong></h4></a></li><p class="collapse text-center" id="collapseInfo-' +
          i +
          '">' +
          ORGINFO[resultList[i]].info +
          '<br><a class ="btn btn-warning btn-lg my-3 mx-auto text-center" href="' +
          ORGINFO[resultList[i]].link +
          '" target="_blank"><strong>Visit Website</strong></a></p>';
    ...
    if (resultList.length > 1) {
      $resultBox.append(
        '<hr class="my-4"><h2><strong> You also matched with:</strong></h2> <ul id="accordion">' +
          resultToString() +
          '</ul><br>'
      );
    }
    $resultBox.append(
      '<hr class="my-4"><form class="text-center form-inline form-row align-items-center form-group-lg input-group input-group-md" name="user-mail" id="mail-form"> <input class="form-control input-lg " aria-label="User email input" type="email" name="email" id="mail" placeholder="Enter email for copy of results"><button class="btn btn-warning btn-md ml-2 font-weight-bold" id="submit" type="submit">Send Results</button></form>'
    );
    $resultBox.append(
      '<hr class="my-4"><p class="text-center">Schedule a consultation with KnightQuest to learn more about your program and many others. Be sure to screenshot your result!</p><p class="text-center"><a class="btn btn-warning btn-lg" href="mailto:kortoutreach@ucf.edu?subject=Schedule KnightQuest Appointment&body=I got ' +
        ORGINFO[resultList[0]].name +
        ' as my result from the OSI Quiz. Could I schedule an appointment to learn more about it and my other options to get involved at UCF?"><strong>Schedule Consultation</strong></a></p>'
    );
  ...
  this._bindEvents = function () {
    ...
    $('.quiz-choice').on('click', function () {
      ...
      $('body, html').animate(
        {
          scrollTop:
            jumboList[parseInt($question.data('quiz-question'))].offset().top -
            25,
        },
        500
      );
