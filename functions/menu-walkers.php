...
class Nav_Menu extends Walker_Nav_Menu { 
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $item_output = sprintf( '%1$s<a%2$s>%3$s%4$s%5$s%6$s</a>%7$s',
            ...
            $args->link_after,
            '<br><span class="subtitle">'.$item->description.'</span>', 
            $args->after
        );
...
class Side_Menu extends Walker_Nav_Menu{
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $attributes .= ! empty( $item->xfn )        ? ' rel="'    . esc_attr( $item->xfn        ) .'"' : '';
        $attributes .= ! empty( $item->url )        ? ' href="'   . esc_attr( $item->url        ) .'"' : '';
        $attributes .= ' class="menu-link list-group-item ' . ( $depth > 0 ? 'sub-menu-link' : 'main-menu-link' ) . '"';
